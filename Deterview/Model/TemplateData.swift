...
//    Question(question: "네트워크 서버에서 트래픽 과부하가 발생할 경우 어떻게 할 것인가?", answer: "네트워크 서버에서 트래픽 과부하가 발생할 경우 어떻게 할 것인가 네트워크 서버에서 트래픽 과부하가 발생할 경우 어떻게 할 것인가 네트워크 서버에서 트래픽 과부하가 발생할 경우 어떻게 할 것인가 네트워크 서버에서 트래픽 과부하가 발생할 경우 어떻게 할 것인가 네트워크 서버에서 트래픽 과부하가 발생할 경우 어떻게 할 것인가 네트워크 서버에서 트래픽 과부하가 발생할 경우 어떻게 할 것인가 네트워크 서버에서 트래픽 과부하가 발생할 경우 어떻게 할 것인가 네트워크 서버에서 트래픽 과부하가 발생할 경우 어떻게 할 것인가 네트워크 서버에서 트래픽 과부하가 발생할 경우 어떻게 할 것인가 네트워크 서버에서 트래픽 과부하가 발생할 경우 어떻게 할 것인가 네트워크 서버에서 트래픽 과부하가 발생할 경우 어떻게 할 것인가 네트워크 서버에서 트래픽 과부하가 발생할 경우 어떻게 할 것인가발생할 경우 어떻게 할 것인가 네트워크 서버에서 트래픽 과부하가 발생할 경우 어떻게 할 것인가 네트워크 서버에서 트래픽 과부하가 발생할 경우 어떻게 할 것인가")]
//
//var iosQuestion: [Question] = [
//    Question(question: "첫번째 질문", answer: "답ㅂㅂㅂㅂ변"),
